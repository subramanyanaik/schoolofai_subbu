"""Turn optimizer_harness.py (jupytext 'percent' format) into optimizer_harness.ipynb.
...
`# %% [markdown]` for prose; markdown cells have their leading `# ` stripped.
    ...
    python tools/build_notebook.py --check     # exit 1 if the committed notebook's code
                                               # differs from optimizer_harness.py (outputs ignored)
"""
import io
import json
import os
...
DST = os.path.join(HERE, "optimizer_harness.ipynb")

BADGE = (
    "[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)]"
    "(https://colab.research.google.com/github/subramanyanaik/schoolofai_subbu/blob/main/"
    "assignment11/optimizer_harness.ipynb)\n"
    "\n"
    "Generated from [`optimizer_harness.py`](optimizer_harness.py) by `tools/build_notebook.py` - "
...
def check():
    ...
    run, but its *code* has to be exactly what optimizer_harness.py says, or the two have drifted.
    """
    ...
    if committed != expected:
        for n in range(max(len(committed), len(expected))):
            ...
            if a != b:
                ...
                break
        print("optimizer_harness.ipynb is out of sync with optimizer_harness.py")
        print("Run: python tools/build_notebook.py")
        return 1
    print(f"optimizer_harness.ipynb is in sync with optimizer_harness.py ({len(expected)} cells)")
    return 0
