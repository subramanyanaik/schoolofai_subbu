...
**Session-5 assignment · every number in this document is computed by the accompanying code, not asserted.**
Run `python3 -m erav5.run_plan` to regenerate. 49/49 internal consistency checks pass.

Inherits from: Session-2 (tokenizer), Session-3 assignment (fertility targets, vocab 196,608), Session-4 assignment (9-stage cleaning pipeline).
...
**Budget derivation (computed, not chosen):**

| Quantity | Value |
|---|---|
| Cluster | 256 GPU × 90 d @ 40% MFU → 7.875×10²³ FLOPs |
| Feasible tokens (`C / 6N_active`) | 8.75T |
...
**Why 4.0T and not 15T.** Three defences, in order of strength:
...
2. **It makes the Indic lane honest.** This is the important one. At 15T, a 20% Indic share needs 3.0T Indic tokens against ~0.54T of real native supply — forcing the lane to 60% translated/synthetic. At 4.0T, the same 20% needs 0.80T, and **real native supply covers 57.5% of it.** The earlier draft's uncomfortable 40/60 native/non-native split was not a data problem; it was an artefact of an inflated budget.
3. **Collecting 8T to train on 4T means the selector can *reject*.** An OPUS-class selector delivered ~6× effective-token value in V4 by discarding. A selector with no surplus to discard is just a bottleneck. 2× collection headroom is what makes selection meaningful rather than decorative.

---

## 2. Capability mixture vs. real supply

Every lane sized against countable supply. `ep` = epochs over unique tokens. `syn%` = share of the lane that must be **generated** because real supply cannot reach it.
...
### 2.1 The loss-mapping correction (Session 5)

The `loss%` column is the one most plans omit, and it is where the sharpest reviewer question lands.

In agentic trajectories, **tool observations are context-only — zero loss.** Only model-generated tokens (plans, code patches, JSON tool calls) receive gradient. Training on tool responses teaches the model to *hallucinate observations*, which is the single worst failure mode for an agent.

So the agentic lane's honest size is not 0.240T. It is **0.084T of supervised tokens** — 35% of its nominal share. Any plan that quotes 6% agentic without this correction is overstating that lane's real training signal by nearly 3×. Across the whole budget, 4.000T of tokens carry **3.844T (96.1%)** of loss.
...
The principle: a majority-synthetic lane is defensible **only** when every generated token passes an independent pass/fail gate that was not itself generated. Answer-checking and execution are such gates. "It looks fluent" is not — which is exactly why the *English* lane is 0% synthetic despite synthetic prose being trivially cheap to produce.

---

## 3. Indic lane — the four-tier split

**20.0% of budget = 0.800T tokens.** No single headline number; the tiers are stated separately as the assignment requires.
...
| synthetic / native ratio | **0.35×** (cap 1.0×) — well inside the 30–50% safe band |

**Tier definitions:**

- **Verified native** — human-authored, script-native, passed the per-language edu classifier *and* ~1% paid native-speaker audit. Sources: Sangraha-verified core (42B), OCR'd books/news with confidence gates (30B), legacy-font rescue from KrutiDev/Shree-Lipi/Bamini (18B). The Sangraha "251B" headline is ~65% machine-translated and is **excluded** from this tier.
- **Unverified native** — script-native but not individually audited: FineWeb2-Indic + broader Sangraha pool (280B), Parliament/judgments/gov records (90B), ASR-mined transcripts (80B).
...
- Verified native runs **exactly 2.0 epochs, total, including the anneal** — the anneal's verified-native component is the *deferred second pass*, carved out of the lane budget, **not** additional exposure. Getting this wrong silently pushes the scarcest tier to ~3× epochs; the validator asserts on it.
...
Global shares are a *weighted average* of stage mixes. If the stages don't integrate back to the global mixture, the plan is internally inconsistent. **S1 is not hand-tuned — it is solved** (`curriculum.solve_s1()`) so integration is exact by construction.
...
|---|---|---|---|---|
| **0 — Direct recall** | 0 | <32 | *"Odisha ki rajdhani kya hai?" → "Bhubaneswar."* | S0–S1 dominant, **never removed** — keeps short answers short |
...
| **4 — Extended / cross-document** | 20+ | 8K–64K | *Read a 40-page judgment + a prior ruling + a Hansard excerpt; identify which precedent controls and why.* | S3–S4 only — requires 256K context to exist first |
...
Band 0 is never dropped from the mixture. A model trained only on long traces loses the ability to answer briefly, which is a real regression, not a cosmetic one.
...
V4 ran an 8% always-on Indic channel. We keep the percentage and **tighten the definition**: translated and synthetic tokens do **not** count.
...
**Explicitly excluded from the reserve:** benchmark test items, eval-adjacent paraphrases, **canary strings**, unverified synthetic CoT.
...
## 7. Fertility — what the Indic budget actually buys

Carried forward unchanged from the Session-3 submission: **vocab 196,608 (3·2¹⁶)**, frozen before pretraining (the one irreversible decision).
...
Tokens are the bill; words are what the model learns. The same budget buys **2.6× the language**.

**Session-2 balance metric** — `Score = 1000 / (X_max − X_min)`:
X_max = 2.15 (Malayalam), X_min = 1.40 (English), spread = 0.75 → **score 1333.3**. The metric rewards *balance*, not just a low average, which is why the Dravidian targets are relaxed per-word (agglutinative morphology packs more meaning into one word) while held per-byte.

---

## 8. Sensitivity — what breaks first if the inventory is wrong
...
- **Verified native misses:** hold exposure at 2.0× (do *not* raise epochs). Backfill from unverified native, which sits at 0.62 epochs with **~0.17T of unused headroom**. Synthetic does **not** expand to fill the gap — the parity cap is a *ratio* to native, so a shrinking native supply shrinks the synthetic ceiling automatically.
- **Code misses:** move to 1.3–1.5× epochs before considering synthetic code. Repetition is safer than generation in a lane where SWE-bench leakage is the dominant risk and where generated code has no correctness signal independent of the tests we also generated.
- **Unverified Indic misses:** most slack of any lane — a 40% cut is fully absorbed at 0.62 epochs. If verified *and* unverified both shrink, the Indic headline share drops below 20% to whatever native supports. **The share follows the supply, not the other way round.**
- **Agentic misses:** no change. The lane was designed around self-play from the start; P3 gates whether self-play works at all, which is the real risk, not supply.

Notably, the floor holds in **every** scenario — because it is defined as a share of the *batch*, not a share of a supply pool that can evaporate.

---

## 9. Cleaning priority — aimed at the starved slots

Applying the Session-4 pipeline yields to the Session-5 mixture: how many **raw** tokens must enter the pipeline to produce the clean tokens each lane needs (including the 2× collection headroom).
...
Verified-native Indic has a **9% yield** — OCR confidence gates plus the native-speaker audit reject hard. It takes **4.0T of raw intake to land 0.18T of usable tokens**, which is the real reason this tier is scarce. It is not that the text doesn't exist; it is that verifying it is expensive.
...
English, forums, and multilingual are **deprioritised** — they have 3×, 3×, and 12× supply headroom respectively. Cleaning more English is effort spent where it changes nothing.

---

## 10. Proxy experiments — the plan as a testable hypothesis

Every ratio here is a hypothesis until a cheap experiment tests it. Each proxy states what would **refute** the plan, not only what would confirm it — a decision rule that can only fire in one direction is not a test.
...
### P5 · Loss-masking on tool observations *(1B, 20B/arm)*
...
### P2 · Protected floor necessity *(1B, 20B/arm)*
...
- **Rule:** keep the floor if B shows a mid-training MILU dip **≥3 pts** that A avoids. **If both curves are flat, drop the floor** — it is unnecessary complexity and it costs selector flexibility.

### P3 · Agentic verification gate *(1B, 20B/arm)*
...
### P1 · Indic synthetic parity cap *(3B, 60B/arm)*
...
- **Rule:** adopt the cap only if A beats B by **≥2.0 pts mean MILU with no chrF++ regression**. **If B wins, the cap rises** — the plan was too conservative and we are leaving Indic capability on the table. If neither separates, drop the cap as unjustified machinery.

### P4 · Mixture-transition gradient stability *(3B, 60B/arm, 3 arms)*
...
- **P4 — mixture-transition gradient stability**, two passes of increasing rigor. Pass 2 (2.73M-param transformer, real prose vs. real Python source, 8 ramp lengths × 3 seeds): an abrupt transition spikes the gradient norm **7.04×**, while *every* non-zero ramp tested — down to 20 steps — sits at 1.4–1.9× with zero loss-spikes. The curve is flat and noisy beyond ~20 steps (the longest ramp is nominally worse than a mid-length one), so nearly all of the protection is captured by a short ramp. Read against this document's own decision rule, that argues for testing a **shorter** blending band than the 100B width, not for confirming it.
...
The plan is machine-checked. `python3 -m erav5.validate` asserts:
...
- **No silent generation:** every synthetic gap is in a lane declared generable, with the volume named
- **Indic:** synthetic ≤ parity with native (0.35× vs 1.0× cap); Indic lane is majority native (57.5%)
- **The epoch trap:** verified-native anneal component is a *subset* of the lane budget (44B ≤ 180B), and total exposure is 2.00× vs a 2.0× cap
...
2. **A smaller budget makes the Indic lane more honest, not less** — 57.5% native at 4.0T versus 40% native at 15T, from the same real supply.
