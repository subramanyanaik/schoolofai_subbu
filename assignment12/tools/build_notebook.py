"""Turn zero_harness.py (jupytext 'percent' format) into zero_harness.ipynb.

No accelerator is requested: this notebook simulates 32 GPUs on CPU threads, and a CUDA
runtime would sit idle while 32 replicas of a model fail to fit on one card.
...
`# %% [markdown]` for prose; markdown cells have their leading `# ` stripped.
    ...
    python tools/build_notebook.py --check     # exit 1 if the committed notebook's code
                                               # differs from zero_harness.py (outputs ignored)
"""
import io
import json
import os
...
DST = os.path.join(HERE, "zero_harness.ipynb")

BADGE = (
    "[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)]"
    "(https://colab.research.google.com/github/subramanyanaik/schoolofai_subbu/blob/main/"
    "assignment12/zero_harness.ipynb)\n"
    "\n"
    "Generated from [`zero_harness.py`](zero_harness.py) by `tools/build_notebook.py` - "
    "edit the `.py`, not this file.\n"
)
...
def check():
    ...
    Outputs are deliberately ignored: the committed notebook keeps the outputs of a real
    run, but its *code* has to be exactly what zero_harness.py says, or the two have drifted.
    """
    ...
    if committed != expected:
        for n in range(max(len(committed), len(expected))):
            ...
            if a != b:
                ...
                break
        print("zero_harness.ipynb is out of sync with zero_harness.py")
        print("Run: python tools/build_notebook.py")
        return 1
    print(f"zero_harness.ipynb is in sync with zero_harness.py ({len(expected)} cells)")
    return 0
