"""Turn truth_harness.py (jupytext 'percent' format) into truth_harness.ipynb.
...
`# %% [markdown]` for prose; markdown cells have their leading `# ` stripped.
    ...
    python tools/build_notebook.py --check     # exit 1 if the committed notebook's code
                                               # differs from truth_harness.py (outputs ignored)
"""
import io
import json
import os
...
DST = os.path.join(HERE, "truth_harness.ipynb")

BADGE = (
    "[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)]"
    "(https://colab.research.google.com/github/subramanyanaik/schoolofai_subbu/blob/main/"
    "assignment10/truth_harness.ipynb)\n"
    "\n"
    "Generated from [`truth_harness.py`](truth_harness.py) by `tools/build_notebook.py` - "
...
def check():
    ...
    run, but its *code* has to be exactly what truth_harness.py says, or the two have drifted.
    """
    ...
    if committed != expected:
        for n in range(max(len(committed), len(expected))):
            ...
            if a != b:
                ...
                break
        print("truth_harness.ipynb is out of sync with truth_harness.py")
        print("Run: python tools/build_notebook.py")
        return 1
    print(f"truth_harness.ipynb is in sync with truth_harness.py ({len(expected)} cells)")
    return 0
